...
describe("Layouts", function()
  ...
  describe("Image layouts", function()
    it("calculates it's layout width and height based on the size of the image", function()
      local img_node = Node({ image = MockLove.newImage(), layout = layouts.image })
    ...
    it("does uses the specified width or height instead of image if specified", function()
      local img_node = Node({ image = MockLove.newImage(), layout = layouts.image, width = 200, height = 250 })
      img_node:layout(parent)
    ...
    it("supports using percentages for the width and height", function()
      ...
      local img_node = Node({ image = MockLove.newImage(), layout = layouts.image, width = "50%", height = "50%" })
      p:add(img_node)
      p:layout()
      assert.equals(100, img_node.box.content.width)
      assert.equals(100, img_node.box.content.height)
  ...
  it("ignores hidden items from layout", function()
    local n = Node({ display = "inline" })
    local c1 = Node({ width = 20, hidden = true })
    local c2 = Node({ width = 25, hidden = false })
    n.parent = nil
    n:add(c1, c2)
    n:layout()
    assert.equals(25, n.box.content.width)
    assert.equals(0, c2.box.x)
  end)
end)
