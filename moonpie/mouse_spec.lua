  ...
  describe("update", function()
    ...
    describe("Click Event", function()
      ...
      it("keeps the same button_down_components even if the mouse moves if the mouse is not released", function()
        ...
        mouse:update(node_tree)
        assert.equals(3, #mouse.button_down_components[1])
        assert.same(node_tree, mouse.button_down_components[1][1])
        assert.same(n2, mouse.button_down_components[1][2])
        assert.same(n3, mouse.button_down_components[1][3])
      end)
