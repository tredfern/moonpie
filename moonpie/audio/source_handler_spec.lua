...
describe("moonpie.audio.source_handler", function()
  ...
  it("can stop all instances", function()
    local audio = sourceHandler:new(mockLove.newAudioClip())
    ...
    audio:play()
    audio:stop()
    assert.is_false(audio.instances[1]:isPlaying())
    assert.is_false(audio.instances[2]:isPlaying())
    assert.is_false(audio.instances[3]:isPlaying())
  end)
end)